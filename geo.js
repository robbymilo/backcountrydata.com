...
for(i=0; i<avyMaster.features.length; i++) {
    ...
    if(avyMaster.features[i].geometry.type == "Polygon") {
        ...
        const center = {
            "coordinates": geolib.getCenter(avyMaster.features[i].geometry.coordinates[0]),
        ...
        console.log(avyMaster.features[i].geometry.type);
        const center = {
            "coordinates": geolib.getCenter(avyMaster.features[i].geometry.coordinates[0][0]),
            "center": avyMaster.features[i].properties.center,
            "name": avyMaster.features[i].properties.name,
            "id": avyMaster.features[i].id,
        };
...
fs.writeFile('./data/static/avy-center-points.json', JSON.stringify(centerPoints), (err) => {  
    if (err) throw err;
