...
module.exports = function(req, res, next, lat, lon) {
    ...
    axios.get(stationLookupUrl).then((response) => {	
        ...
        var discussionLookupUrl =  'https://www.wrh.noaa.gov/total_forecast/getprod.php?afos=xxxafd' + nwsOffice + '&xml';
        result['discussionLookupUrl'] = discussionLookupUrl;

        console.log('finding discussion');
        axios.get(discussionLookupUrl).then((response) => {	
            ...
            var forecastLookupUrl = 'https://forecast.weather.gov/MapClick.php?lat='+ lat +'&lon='+ lon +'&FcstType=json';
            result['forecastLookupUrl'] = forecastLookupUrl; 
        
            console.log('finding forecast');
            axios.get(forecastLookupUrl).then((response) => {	
